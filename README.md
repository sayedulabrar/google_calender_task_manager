# Features
1. can create tasks using create new task and update, delete task by clicking on the task and making changes.
2. can select Drag factor to decide how much task can be dragged in timeline in Day view. We can then drag it and it will update when we remove click. For Week view we can also do the same and drag between days too. 
3. At the top left we can see a filter . The calender will appear based on it. today button only available for Day View. 
4. We can search tasks and it will auto shrink timeline for better ux . Normally without search field usage it will show full timeline from 12 am to 11 pm . 
6. In year view we can see how much tasks as tooltip.
7. In month view only 2 tasks will appear and the rest as +n more tasks.
8. In day and week view we show all tasks as there is drag feature. 


# Running 
use `docker compose up --build` to run and `docker compose down -v` to down and up again normally. Wait for some time to finish the build process and hit `http://localhost:5173/` for viewing the work. 